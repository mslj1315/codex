...
class RetrofitStoryboardVideoApi(private val api: StoryboardWireApi) : StoryboardVideoApi {
    override suspend fun createUploadGrant(storeId: String, taskId: String, shotListId: String, source: GalleryVideo): UploadGrant { val m=api.grant(storeId,taskId,shotListId,mapOf("expectedSizeBytes" to source.sizeBytes,"contentType" to "video/mp4")); return UploadGrant(m["assetId"] as String, m["uploadUrl"] as String, m["expiresAt"] as String) }
    override suspend fun completeUpload(storeId: String, taskId: String, shotListId: String, assetId: String): UploadedAsset { val m=api.complete(storeId,taskId,shotListId,assetId); return UploadedAsset(m["assetId"] as String, "Accepted video", (m["durationSeconds"] as? Number)?.toInt() ?: 0) }
    ...
    override suspend fun selectCover(storeId:String,taskId:String,shotListId:String,projectId:String,renderId:String,candidateId:String,title:String)=api.cover(storeId,taskId,shotListId,projectId,renderId,mapOf("candidateId" to candidateId,"title" to title)).render()
}
