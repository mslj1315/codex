...
class RetrofitStoryboardVideoApi(private val api: StoryboardWireApi) : StoryboardVideoApi {
    ...
    override suspend fun saveProject(storeId:String,taskId:String,shotListId:String,draft:StoryboardDraft,finalize:Boolean)=api.save(storeId,taskId,shotListId,draft.projectId,mapOf("slots" to draft.slots.map { slot -> linkedMapOf<String, Any?>("slotId" to slot.id,"kind" to slot.kind,"assetId" to slot.assetId,"order" to slot.order,"trimStartSeconds" to slot.trimStartSeconds,"trimEndSeconds" to slot.trimEndSeconds,"muted" to slot.muted,"subtitleEnabled" to slot.subtitleEnabled,"subtitleText" to slot.subtitleText).also { if (slot.kind == "shot") it["shotIndex"] = slot.shotIndex } },"coverAssetId" to draft.coverAssetId,"coverFrameOffsetSeconds" to draft.coverFrameOffsetSeconds,"coverTitle" to draft.coverTitle,"finalize" to finalize)).draft()
...
class ContentResolverDirectVideoUploader(private val resolver: ContentResolver, private val client: OkHttpClient = OkHttpClient()) : DirectVideoUploader { override suspend fun upload(grant: UploadGrant, source: GalleryVideo) = withContext(Dispatchers.IO) { val body = object : RequestBody() { override fun contentType(): MediaType? = MediaType.parse("video/mp4"); override fun contentLength() = source.sizeBytes; override fun writeTo(sink: BufferedSink) { resolver.openInputStream(Uri.parse(source.id))!!.use { input -> Okio.source(input).use(sink::writeAll) } } }; client.newCall(Request.Builder().url(grant.uploadUrl).put(body).build()).execute().use { check(it.isSuccessful) } } }
private fun Map<String,Any>.draft(): StoryboardDraft { val slots=(this["slots"] as? List<Map<String,Any>>).orEmpty().map { StoryboardSlot(it["slotId"] as String,(it["order"] as Number).toInt(),it["kind"] as String, it["assetId"] as? String,(it["trimStartSeconds"] as? Number)?.toInt()?:0,(it["trimEndSeconds"] as? Number)?.toInt()?:0,it["muted"] as? Boolean?:false,it["subtitleEnabled"] as? Boolean?:true,it["subtitleText"] as? String?:"",(it["shotIndex"] as? Number)?.toInt()) }; return StoryboardDraft(this["id"] as String,(this["version"] as Number).toInt(),slots,this["coverAssetId"] as? String,(this["coverFrameOffsetSeconds"] as? Number)?.toInt(),this["coverTitle"] as? String?:"",this["status"] as? String?:"draft") }
