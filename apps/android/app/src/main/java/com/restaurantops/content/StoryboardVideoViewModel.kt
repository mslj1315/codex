...
class StoryboardVideoViewModel(private val repository: StoryboardVideoRepository) : ViewModel() {
    ...
    fun requestRender(storeId: String, taskId: String, shotListId: String, kind: String) = viewModelScope.launch { val draft = state.draft ?: return@launch; runCatching { val finalized = repository.saveProject(storeId, taskId, shotListId, draft, finalize = true); state = state.copy(draft = finalized); repository.createRender(storeId, taskId, shotListId, finalized.projectId, kind) }.onSuccess { state = state.copy(renders = listOf(it) + state.renders) }.onFailure { state = state.copy(error = "Finalize the storyboard before rendering.") } }
