...
class OperationsScreenTest {
    @Test
    fun `formats missing readiness metrics with catalog labels and neutral fallback`() {
        val labels = mapOf(
            "revenue" to "营业额（元）",
            "average_spend" to "客单价（元）"
        )

        assertEquals("缺少指标：营业额（元）、未命名指标、客单价（元）", readinessMissingMetricsText(listOf("revenue", "unknown_metric", "average_spend"), labels))
        assertEquals(null, readinessMissingMetricsText(emptyList(), labels))
    }

    @Test
    fun `formats diagnostic evidence with catalog presentation and neutral fallback`() {
