...
The first release supports one internal role:

- `operator_admin`: create, edit, preview, publish, and disable template and
  rule versions.
...
operator accounts and assigns the `operator_admin` role. Disabling an account
invalidates its active sessions. Logout revokes the current session.
...
3. An audit event records draft creation, draft update, publish, disable, and
   related actor, role, time, reason, and version information.
...
Lifecycle states are `draft`, `published`, and `disabled`. An operator can
save a draft, publish it directly, or disable a published version. Updating a
published item starts a new draft version. There is no submit, return, review
inbox, or second-person approval flow in the first release. Only `published`
versions participate in template matching and review-rule selection.
...
- operator audit fields plus lifecycle status.
...
Sign in
  -> Template library
      -> list, filters, version detail, draft editor, publish, disable
  -> Review rules
      -> list, filters, version detail, rule editor, test preview, publish, disable
```

The operator sees both template and rule authoring flows, including published
version history. The UI uses role-derived capability flags from the session
response, not caller-supplied role values. It renders content as ordinary text
and never uses unsafe HTML.
...
API tests prove account/session lifecycle, CSRF enforcement, operator
authorization, neutral provider forbidden responses, version increments,
published immutability, disabling, published-only selection, rule preview,
and malformed-input handling. Migration tests exercise full migration order
and PostgreSQL append-only triggers where supported.

Console tests prove login/logout/session expiry, role-gated navigation,
draft/publish/disable flows, safe text rendering, request allowlisting, CSRF
