...
function validateStage(input: OperatingStageInput): void { if (!/^\d{4}-\d{2}-\d{2}$/.test(input.effectiveDate)) throw new ProfileValidationError("effectiveDate is required"); if (!input.primaryGoal?.trim()) throw new ProfileValidationError("primaryGoal is required"); if (input.secondaryGoal?.trim() === input.primaryGoal.trim()) throw new ProfileValidationError("secondaryGoal must differ from primaryGoal"); }
