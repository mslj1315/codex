...
export interface CopyReviewResult { approved: boolean; findings: CopyReviewFinding[]; }
export class CopyReviewService {
  constructor(private readonly database:Database) {}
  async review(taskId:string,copyId:string,text:string):Promise<CopyReviewResult> {
    ...
    await this.database.query("INSERT INTO content_task_copy_reviews(id,task_id,copy_id,rule_snapshot_json,result_json,approved) VALUES($1,$2,$3,$4,$5,$6)",[randomUUID(),taskId,copyId,JSON.stringify(snapshot),JSON.stringify({approved,findings}),approved]);
