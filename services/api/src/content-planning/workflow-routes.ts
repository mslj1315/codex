...
export async function registerWorkflowRoutes(app: FastifyInstance, database: Database, resolve: (request: FastifyRequest) => Promise<TrustedContext | undefined>, generator?: ModelGenerationService): Promise<void> {
  ...
  app.get("/v1/stores/:storeId/content-tasks/:id", async request => {
    ...
    const copies = await database.query<Row>("SELECT c.id,c.topic_id,c.title,c.body,c.strategy,c.product_reference,c.goal_reference,c.commercial_level,c.version,c.status FROM content_task_copies c JOIN content_task_topics t ON t.id=c.topic_id WHERE c.task_id=$1 ORDER BY t.position,c.position", [item.id]);
