...
export class ProviderFeedbackRepository {
  ...
  async list(input: ProviderFeedbackListInput): Promise<ProviderFeedbackPage> {
    ...
    try {
      ...
      const cutoff = new Date(input.now.getTime() - 30 * 24 * 60 * 60 * 1000);
      const values: unknown[] = [cutoff];
      const filters: string[] = [];
      if (input.activityState) { values.push(input.activityState); filters.push(`activity_state = $${values.length}`); }
      if (input.readinessState) { values.push(input.readinessState); filters.push(`readiness_state = $${values.length}`); }
      if (input.after?.lastSuccessfulImportAt) {
        values.push(input.after.lastSuccessfulImportAt, input.after.enterpriseId, input.after.storeId);
        const timestamp = values.length - 2; const enterprise = values.length - 1; const store = values.length;
        filters.push(`(last_successful_import_at < $${timestamp} OR last_successful_import_at IS NULL OR (last_successful_import_at = $${timestamp} AND (enterprise_id > $${enterprise} OR (enterprise_id = $${enterprise} AND store_id > $${store}))))`);
      } else if (input.after) {
        values.push(input.after.enterpriseId, input.after.storeId);
        const enterprise = values.length - 1; const store = values.length;
        filters.push(`(last_successful_import_at IS NULL AND (enterprise_id > $${enterprise} OR (enterprise_id = $${enterprise} AND store_id > $${store})))`);
      }
      ...
      const result = await client.query<Row>(`WITH scopes AS (
        ...
        FROM action_cards GROUP BY enterprise_id, store_id
      ), projected AS (SELECT s.enterprise_id, s.store_id, i.imported_at AS last_successful_import_at, v.confirmed_at AS last_confirmed_at,
          ...
          CASE WHEN i.imported_at IS NULL THEN 'inactive' WHEN i.imported_at >= $1 THEN 'active' ELSE 'stale' END AS activity_state,
          CASE WHEN v.confirmed_at IS NULL THEN 'unavailable' WHEN COALESCE(c.covered_count, 0) >= (SELECT required_count FROM required) THEN 'ready' ELSE 'incomplete' END AS readiness_state
        ...
        SELECT * FROM projected ${where}
        ORDER BY last_successful_import_at DESC NULLS LAST, enterprise_id ASC, store_id ASC LIMIT $${values.length}`, values);
...
function toFeedbackRow(row: Row, now: Date): ProviderFeedbackRow {
  const imported = dateOrNull(row.last_successful_import_at); const confirmed = dateOrNull(row.last_confirmed_at);
  ...
  return { enterpriseId: String(row.enterprise_id), storeId: String(row.store_id), lastSuccessfulImportAt: imported, lastConfirmedAt: confirmed,
    activityState: row.activity_state as ActivityState, readinessState: row.readiness_state as ReadinessState, missingMetricCount: missing,
