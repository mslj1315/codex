...
export class DatabaseRenderWorkerRepository implements RenderWorkerRepository {
  ...
  async claim(): Promise<ClaimedRender | undefined> {
    const client = await this.database.connect(); try { await client.query("BEGIN"); const now = this.now();
      ...
      const assets = await client.query<Row>("SELECT a.object_key,v.slots_json FROM storyboard_media_assets a JOIN storyboard_project_versions v ON v.project_id=a.project_id AND v.version=$2 WHERE a.project_id=$1 AND a.status='accepted' AND a.expires_at>$3", [job.project_id, job.project_version, now]); const slots = slotsArray(assets.rows[0]?.slots_json);
...
function slotsArray(value: unknown): Array<{ assetId?: string; subtitleText?: string }> { const parsed: unknown = typeof value === "string" ? JSON.parse(value) : value ?? []; if (!Array.isArray(parsed)) throw new Error("Stored storyboard slots are invalid"); return parsed as Array<{ assetId?: string; subtitleText?: string }>; }
