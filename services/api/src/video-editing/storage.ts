...
class InternalSignerVideoStorage implements VideoStorage, ProtectedRenderStorage {
  ...
  private async call<T>(action: string, body: unknown): Promise<T> { let response: Response; try { response = await fetch(`${this.endpoint.replace(/\/$/, "")}/${action}`, { method: "POST", headers: { authorization: `Bearer ${this.token}`, "content-type": "application/json" }, body: JSON.stringify(body) }); } catch { throw Object.assign(new Error("video storage signer request failed"), { code: "ECONNRESET" }); } if (!response.ok) throw Object.assign(new Error("video storage signer request failed"), { code: "ETIMEDOUT" }); return response.status === 204 ? undefined as T : await response.json() as T; }
}
