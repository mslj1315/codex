...
describe("model providers", () => {
  it("normalizes DeepSeek requests and returns provider/model/usage metadata", async () => {
    const http: ModelHttpClient = { post: vi.fn().mockResolvedValue(successfulResponse([{ title: "午市排队的原因", angle: "后厨备料", productReference: "招牌牛肉面", goalReference: "到店" }])) };
    const service = createGenerationService({ provider: "deepseek", model: "deepseek-chat", apiKey: "deepseek-secret", maxRetries: 0 }, { http });

    const result = await service.generateStructured({ requestId: "request-1", promptVersion: "topic-v1", input: { inspiration: "早上熬汤" }, schema: topicArraySchema });
  ...
  it("normalizes Qwen requests without changing the configured provider", async () => {
    const http: ModelHttpClient = { post: vi.fn().mockResolvedValue(successfulResponse([{ title: "师傅备料", angle: "手艺", productReference: "米线", goalReference: "认知" }])) };
    const service = createGenerationService({ provider: "qwen", model: "qwen-plus", apiKey: "qwen-secret", maxRetries: 0 }, { http });

    await service.generateStructured({ requestId: "request-2", promptVersion: "topic-v1", input: { inspiration: "现熬" }, schema: topicArraySchema });
  ...
  it("retries a transient failure only with the same configured provider", async () => {
    const http: ModelHttpClient = { post: vi.fn().mockRejectedValueOnce(new Error("temporary upstream failure")).mockResolvedValueOnce(successfulResponse([{ title: "午市", angle: "过程", productReference: "米线", goalReference: "到店" }])) };
    ...
    const result = await service.generateStructured({ requestId: "request-3", promptVersion: "topic-v1", input: {}, schema: topicArraySchema });
  ...
  it("aborts a timed-out request and does not retry beyond its configured bound", async () => {
    ...
    await expect(service.generateStructured({ requestId: "request-4", promptVersion: "topic-v1", input: {}, schema: topicArraySchema })).rejects.toThrow("timed out");
  ...
  it("rejects malformed model output before it can become a structured result", async () => {
    ...
    await expect(service.generateStructured({ requestId: "request-5", promptVersion: "topic-v1", input: {}, schema: topicArraySchema })).rejects.toThrow("structured output");
  });

  it("enforces future copy and shot-list safety bounds", () => {
    expect(() => copyDraftsSchema.parse([{ title: "one", body: "a", strategy: "story", productReference: "米线", goalReference: "到店" }, { title: "two", body: "b", strategy: "expert", productReference: "米线", goalReference: "到店" }, { title: "three", body: "c", strategy: "life", productReference: "米线", goalReference: "到店" }])).not.toThrow();
    expect(() => copyDraftsSchema.parse([{ title: "same", body: "a", strategy: "story", productReference: "米线", goalReference: "到店" }, { title: "same", body: "b", strategy: "story", productReference: "米线", goalReference: "到店" }, { title: "same", body: "c", strategy: "story", productReference: "米线", goalReference: "到店" }])).toThrow("materially distinct");
    expect(() => shotListSchema.parse([{ order: 0, shot: "汤锅", durationSeconds: 3, narration: "现熬", productReference: "米线", goalReference: "到店" }])).toThrow("safe integer");
  });
